...
import {
    OBTENER_FORMULAS_BASE,
    DELETE_FORMULA_BASE
} from '../../../services/FormulaBaseService'
import { Redirect, Link } from 'react-router-dom';
const { Column, HeaderCell, Cell, Pagination } = Table;

const FormulasBase = ({ ...props }) => {
    const [page, setPage] = useState(1);
    const [displayLength, setDisplayLength] = useState(10);
    const [confimation, setConfirmation] = useState(false);
    const [filter, setFilter] = useState('');
    const { loading, error, data: formulas } = useQuery(OBTENER_FORMULAS_BASE, { pollInterval: 1000 });
    const [desactivar] = useMutation(DELETE_FORMULA_BASE);
    ...
    const onDeletObjeto = async (id) => {
        ...
        const { estado, message } = data.desactivarFormulaBase
        if (estado) {
            Notification['success']({
                title: 'Eliminar Fórmula Base',
                duration: 5000,
                description: message
            })
        } else {
    ...
    const getData = () => {
        return formulas.obtenerFormulasBase.filter((value, index) => {
            if (filter !== "") {
        ...
        <>
            ...
            <div className="mt-3">
                                        ...
                                        return (
                                            <>
                                                <div className="d-inline-block mx-2">
                                                    <Link to={`/config/formulasbase/editar/` + rowData.id}><Action tooltip="Editar" icon="edit" color="orange" size="xs" /></Link>
                                                </div>
                                                <div className="d-inline-block mx-2">
                                                    <Action tooltip="Eliminar" icon="trash" color="red" onClick={() => setConfirmation({ bool: true, id: rowData.id })} size="xs" />
                                                </div>
                                                <div className="d-inline-block mx-2">
                                                    <Link to={`/config/formulasbase/detalles/` + rowData.id}><Action tooltip="Detalles" icon="info" color="blue" size="xs" /></Link>
                                                </div>
                                            </>
                                        );
                                    }
                ...
                <Pagination
                    ...
                    total={formulas.obtenerFormulasBase.length}
                    onChangePage={handleChangePage}
                    onChangeLength={handleChangeLength}
                />
            </div>
            <div className="d-flex justify-content-start">
                <Link to="/config/formulasbase/nuevo"><Boton name="Nueva Fórmula" tooltip="Nueva Fórmula" color="green" icon="plus" size="sm" position="end" /></Link>
            </div>
            {isConfirmation}
        </>
    )
...
export default withRouter(FormulasBase)