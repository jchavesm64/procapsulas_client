...
import { Notification, Input, InputPicker, Loader } from 'rsuite'
import Boton from '../shared/Boton'
import { UPDATE_INSUMO } from '../../services/InsumosService'
import { OBTENER_UBICACIONES } from '../../services/UbicacionService'

const FormularioInsumo = ({ props, insumo }) => {
    const [descripcion, setDescripcion] = useState(insumo.descripcion)
    const [codigo, setCodigo] = useState(insumo.codigo)
    const [area, setArea] = useState(insumo.area.id)
    const [cantidad_limite, setCantidadLimite] = useState(insumo.cantidad_limite)
    const [insertar] = useMutation(UPDATE_INSUMO);
    ...
    useEffect(() => {
        ...
        setArea(insumo.area.id)
        setCantidadLimite(insumo.cantidad_limite)
    }, [insumo])
    ...
    const onSaveInsumo = async () => {
        try {
            if (cantidad_limite > 0) {
                ...
                const { data } = await insertar({ variables: { id: insumo.id, input }, errorPolicy: 'all' });
                console.log(data)
                const { estado, message } = data.actualizarInsumo;
                if (estado) {
                ...
                Notification['warning']({
                    title: 'Actualizar Insumo',
                    duration: 5000,
                    description: "La cantidad limite en stock debe ser mayor a cero"
                })
            }
        } catch (error) {
            console.log(error)
    ...
    return (
        <>
            <div>
                ...
                <h3 className="text-center">Editar Insumo</h3>
                <div className="row">
...
export default withRouter(FormularioInsumo)